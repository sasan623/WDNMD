		...
		try {
			...
			if (op.equals("add")) {
				...
				if (list.contains(pid)) {//如果链表里有，那说明该商品已经存在于购物车，在数量上加1即可
					...
					sql = "update `shopcart` set cart_product_quantity = ?, totalprice = ?  where product_id=?;";
					stmt = conn.prepareStatement(sql);
					stmt.setInt(1, num);
					stmt.setInt(2, totalprice);
					stmt.setInt(3, pid);
					stmt.executeUpdate();
					out.print("p_id = cart_id");
					response.sendRedirect("buy.jsp");//重定向到购物车
					stmt.close();
					conn.close();
				} else {//没有购物车匹配不到该商品则直接插入
					sql = "insert into `shopcart`(product_id,product_name,product_price,cart_product_quantity,totalprice) values(?,?,?,?,?);";
					stmt = conn.prepareStatement(sql);
